...
.NOTES
    Code is a derivative work from work written by Bernie Salvaggio
    (berniesalvaggio(at)gmail(dot)com) under the MIT license.
    Modified and adapted by Kyle Elliott (kelliott(at)ciotech(dot)us)
...
$MailMessage.from = ($ServerName + "@domain.com")
$MailMessage.To.add("user@domain.com")
$MailMessage.Subject = $ServerName + " Log Archive Results"
...
$MailMessageDetermination = "never"
...
Switch($ArchiveGrouping) {
    ...
    Default {
        Write-Log "Invalid Archive Grouping selected. You selected '$ArchiveGrouping'. Select month or day."
        $ErrorTrackerEmail = $true
        Send-Email
        Exit
    }
...
if ($7zVersion -lt 15) {
    Write-Log "Error: 7-Zip version 15 or higher is required. You are running version $7zVersion."
    $ErrorTrackerEmail = $true
    Send-Email
    Exit
}
...
if ($ArchiveStorage) {
    if (!(Test-Path $ArchiveStorage) -and ($ArchiveStorage -ne "")) {
            ...
            Please create the requested folder."
        $ErrorTrackerEmail = $true
        Send-Email
        Exit
    }
